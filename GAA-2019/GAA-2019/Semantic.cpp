	...
	bool Semantic(LT::LexTable& lt, IT::IdTable& it, Log::LOG& log)
	{
		...
		for (int i = 0; i < lt.size; i++)
		{
			...
			case LEX_DIRSLASH:
			{
				...
				{
					if (lt.table[i -1].lexema == LEX_LITERAL && ((it.table[lt.table[i-1].idxTI].si == IT::Ten && it.table[lt.table[i-1].idxTI].value.vint.Ten == 0) || (it.table[lt.table[i-1].idxTI].si == IT::Hex && it.table[lt.table[i-1].idxTI].value.vint.Hex == "00")))
					{
						sem_ok = false;
